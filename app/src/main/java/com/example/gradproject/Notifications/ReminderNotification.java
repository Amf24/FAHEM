...
public class ReminderNotification extends BroadcastReceiver
{
    ...
    public void onReceive(final Context context, Intent intent)
    {
        ...
        MainActivity.appointmentNotificationReminder = true; //This is to change the static variable in MainActivity inorder to show appointments list if clicked on notification.

        getTheUserName();

        appointmentNotificationIntent = new Intent(context, MainActivity.class); // AppointmentFragment / AppointmentDetailsActivity / MainActivity

        //appointmentNotificationIntent.putExtras(ActivitySessionDetails.appointmentDetailsIntent.getExtras());

        TaskStackBuilder stackBuilder = TaskStackBuilder.create(context);
        stackBuilder.addParentStack(MainActivity.class); // AppointmentFragment / AppointmentDetailsActivity / MainActivity
        stackBuilder.addNextIntent(appointmentNotificationIntent);
        ...
        handler.postDelayed(new Runnable()
        {
            ...
            public void run()
            {
                ...
                if(userName != null)
                    notificationMessage = "Hi " + userName +", " + "This notification is to remind you of your appointment."; // This if the app in background.
                else
                    notificationMessage = "Hi, " + "This notification is to remind you of your appointment."; // This if the user close the app.
    ...
    public void getTheUserName()
    {
        ...
        {
            Query query = tutorsDatabase.orderByChild("accountId").equalTo(userId).limitToFirst(1);
            ...
            query.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                {
                    if(dataSnapshot.exists())
                    {
                        Tutor s = dataSnapshot.getChildren().iterator().next().getValue(Tutor.class);
                        if(s != null)
                            userName = s.getFullName();
                    }
                }
